...
# colors
. ./vendor/blackout/tools/colors

if [ ! -d ".repo" ]; then
	echo -e ${red}"No .repo directory found.  Is this an Android build tree?"${txtrst}
	exit 1
fi
if [ ! -d "vendor/blackout" ]; then
	echo -e ${red}"No vendor/blackout directory found.  Is this a Blackout ROM build tree?"${txtrst}
	exit 1
fi
...
# get current version
eval $(grep "^BLK_VERSION_" vendor/blackout/config/blk_common.mk | sed 's/ *//g')
VERSION="$BLK_VERSION_MAJOR.$BLK_VERSION_MINOR.$BLK_VERSION_MAINTENANCE"

echo -e ${cya}"Building ${bldgrn}Blackout ROM ${bldylw}v$VERSION"${txtrst}
...
# squisher
vendor/blackout/tools/squisher
